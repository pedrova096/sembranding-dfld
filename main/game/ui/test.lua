...
local fallback_cam = {
  ...
  orthographic = true,
  scaleMode = SCALEMODE_EXPANDVIEW,
  orthoScale = 1,
  useViewArea = false,
  viewArea = vmath.vector3(960, 640, 0),
  halfViewArea = vmath.vector3(480, 320, 0),
...
local curCam =
    fallback_cam   -- current camera data table, defaults and resets to `fallback_cam` if no user camera is active
...
function M.camera_init(cam_id, data)
  if cameras[cam_id] then
    print("ERROR: rendercam.camera_init() - Camera name conflict with ID: " ..
...
-- Wrapper follow that stores bounds separately from the default follow state.
function M.new_follow_with_bounds(target_id, allowMultiFollow, bounds, cam_id)
  local cam = cam_id and cameras[cam_id] or curCam
  cam.follows_with_bounds = cam.follows_with_bounds or {}

  if allowMultiFollow then
    table.insert(cam.follows_with_bounds, target_id)
  else
    cam.follows_with_bounds = { target_id }
  end

  cam.following_with_bounds = true

  if not bounds then
    cam.follow_bounds = nil
    return
  end
...
function M.new_unfollow_with_bounds(target_id, cam_id)
  local cam = cam_id and cameras[cam_id] or curCam
  cam.follows_with_bounds = cam.follows_with_bounds or {}

  for i, v in ipairs(cam.follows_with_bounds) do
    if v == target_id then
      table.remove(cam.follows_with_bounds, i)
      if #cam.follows_with_bounds == 0 then
        cam.following_with_bounds = false
      end
      return
    end
  end
...
function M.new_set_follow_bounds(bounds, cam_id)
  local cam = cam_id and cameras[cam_id] or curCam
  if not bounds then
    cam.follow_bounds = nil
    return
...
function M.new_get_follow_bounds(cam_id)
  local cam = cam_id and cameras[cam_id] or curCam
  return cam.follow_bounds
end

function M.new_is_following_with_bounds(cam_id)
  local cam = cam_id and cameras[cam_id] or curCam
  return cam.following_with_bounds == true
end

function M.new_get_follows_with_bounds(cam_id)
  local cam = cam_id and cameras[cam_id] or curCam
  cam.follows_with_bounds = cam.follows_with_bounds or {}
  return cam.follows_with_bounds
end

function M.new_follow_lerp_func(curPos, targetPos, dt, cam_id)
  local cam = cam_id and cameras[cam_id] or curCam
  local bounds = cam.follow_bounds
  if bounds then
    targetPos = vmath.vector3(
      clamp(targetPos.x, bounds.min_x, bounds.max_x),
      clamp(targetPos.y, bounds.min_y, bounds.max_y),
      targetPos.z
    )
  end
  return vmath.lerp(dt * M.follow_lerp_speed, curPos, targetPos)
...
function M.update_window_size(x, y)
  ...
  M.viewport.width = x; M.viewport.height =
      y -- if using a fixed aspect ratio this will be immediately overwritten in M.update_window
end
...
function M.update_window(newX, newY)
  if curCam then
    ...
    local x, y = get_target_worldViewSize(curCam, curCam.viewArea.x, curCam.viewArea.y, M.window.x, M.window.y, newX,
      newY)
    ...
    for i, v in ipairs(listeners) do
      msg.post(v, "window_update",
        { window = M.window, viewport = M.viewport, aspect = curCam.aspectRatio, fov = curCam.fov })
    end
  end
end
...
function M.screen_to_world_2d(x, y, delta, worldz, raw)
  ...
  local t = (worldz - curCam.abs_nearZ) /
      (curCam.abs_farZ - curCam.abs_nearZ) -- normalize desired Z to 0-1 from abs_nearZ to abs_farZ
